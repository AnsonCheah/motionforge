...
### 2.11 As-built deviations (curobov2 reality)
The idealized spec met a few hard edges in this cuRobo build; SPEC §9.1 is the authoritative list. The load-bearing ones: **`plan_grasp` is broken** (fails to converge and corrupts planner state) so the pick is a per-segment `plan_pose` chain like the place; **voxel layers must be batch-loaded** (`VoxelData.load_batch`), because the `Scene` voxel path replaces instead of composing — the earlier "merge layers with `torch.minimum`" idea is retired; the **tool TCP / held object / gripper geometry are injected robot-config links** (the tool frame is the TCP, not the flange), and the held object attaches frame-correctly at the TCP (a prior identity-offset bug left it a phantom); **linear-approach axis selection happens in the goal frame**; and **CUDA graphs stay on** because mutations replay in-place (measured ~30 ms/plan, no per-mutation re-capture). `min_clearance_m` as a separate hard ESDF gate is deferred — cuRobo's success already enforces margin-level collision freedom.
