...
- **In scope (MVP) — BUILT:** the `CellConfig` data model (Section 4) and its loader (`motionforge/cell_config.py`; example `configs/cell_example.yml`). The artifact is a thin **YAML manifest + referenced geometry assets**: the manifest holds the semantic, human-diffable fields (camera roles, robot mount, ROIs, transforms, tool library); geometry (mesh/USD) is referenced by path. Loaded once at node startup; the static collision world, camera registry, ESDF grid spec, and tool library all derive from it instead of being assembled in code (§5.1/§5.2/§5.4). Gripper `collision_geom_fn`s (not YAML-serializable) are resolved from a named-builder registry. The node warns loudly when no cell config / no static bodies are present (empty static world is a safety risk — CONTEXT §3).
...
## 9.1 Implementation deviations & decisions (as built)

Where the implementation diverges from the idealized spec above, and why. These are verified against the curobov2 build in `external/curobo`, not its docs.

- **Pick uses per-segment `plan_segment` chains, not `plan_grasp` (§3, §5.3, §6).** cuRobo's `plan_grasp` is broken in this build: its internal linear approach→grasp step fails to converge, and a failed call **corrupts planner state** so later goalset plans return `None`. The coordinator plans the pick as approach→grasp→retract `plan_pose` segments (the same path as place). `MotionPlannerAdapter.plan_grasp` remains for the API tests but warns unless `acknowledge_broken=True`.
- **ESDF layers compose via one batched `load_batch`, not via the `Scene` (§5.2).** cuRobo's `Scene(voxel=[a, b])` path *replaces* rather than composes (last-grid-wins). `CollisionWorldManager.commit()` pushes static cuboids/meshes through `update_world`, then batch-loads all ESDF layers via `MotionPlannerAdapter.load_voxel_layers` → `VoxelData.load_batch`, which composes grids at their own poses. All ROI layers share dims/voxel_size (the cache is sized once); poses differ. Single source of truth: `motionforge/collision/grid_spec.py` (`ESDFGridSet`/`ROIGridSpec`).
- **Tool TCP is the planned frame via an injected `tcp` link (§5.4, §8).** Vision emits TCP poses, so a fixed `tcp` link (parent `tool0`, transform = the active tool's `tcp_pose`) is injected into the robot config and set as the sole `tool_frames` entry. The held workpiece (`attached_object`) and the gripper's width-dependent geometry (`tool_collision`) are separate injected links with self-collision-ignore pairs. The transform is immutable, so a tool change with a different TCP requires a planner rebuild (acceptable — tool-change motion is deferred).
- **Held-object / tool attach is frame-correct (§5.2, §5.4).** Workpiece obstacles carry their base-frame world pose and are attached with an identity `world_objects_pose_offset` so cuRobo resolves their spheres into the tool frame (they land at the TCP and ride rigidly); tool geometry is authored in the TCP frame and attached with no offset. A lone fitted sphere is duplicated to dodge a cuRobo single-sphere edge case.
- **Linear-approach axis is chosen in the GOAL frame (§5.3).** cuRobo's `ToolPoseCriteria.linear_motion(axis=…)` (with `project_distance_to_goal=True`) weights axes in the goal frame, so the base-frame `approach_axis` is rotated into the goal frame before the principal-axis pick — otherwise a rotated grasp's straight-line approach bends.
- **`min_clearance_m` hard filter is deferred (§8, §11).** cuRobo's `success` already enforces a collision-free trajectory within the optimizer's activation margin; a dedicated ESDF clearance query is a later refinement, not implemented this round.
- **CUDA graphs stay ON with reset plumbing (§8, §11).** Runtime mutations (world/voxel updates, attach/detach, per-segment criteria swaps) write in-place into pre-allocated tensors, so captured graphs replay against them — measured visible without re-capture, and per-segment plan time stays ~30 ms (RTX 5080). `MotionPlannerAdapter.reset_cuda_graphs()` + `GRAPH_RESET_KINDS` (empty) are the safety switch if a kind is ever found stale.
- **Execution carries planned timing (§5.6).** Each controller `Waypoint` includes `dt_s` (planned time delta from cuRobo's parameterization, preserved through down-sampling); `speed`/`zone` are reserved for the real RAPID mapping. After each segment the coordinator verifies joint feedback against the planned end (`exec_joint_tol_rad`) and faults on divergence, empty `q0`, or any exception — running best-effort cleanup (stop + detach) so the next cycle starts clean.
- **Self-filter is real via cuRobo `RobotSegmenter` (§5.1, §5.2).** `motionforge/perception/self_filter.py` masks the robot's own body out of a base-frame depth observation before ESDF integration.
